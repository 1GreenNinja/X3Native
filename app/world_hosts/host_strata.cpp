// --world strata host — the STRATA geological descent (facility base Y=0 ->
// Club 1127 Y=-200). Lifted from feat/strata-descent's inline main() block into
// the #28 world-host registry; the only edits are reaching shared state via the
// HostContext (same mechanical mapping as the other host TUs).
    ...
    if (worldMode == "strata") {
        ...
        if (headless) {
            ...
            const std::string outPath = screenshot ? screenshotPath
                                                   : std::string("captures/agent_strata.png");
