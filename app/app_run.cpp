...
#include "scene.h"
#include "engine_console.h"                  // D-CONSOLE fold: the shared cvar/command registry (also used by HostShell)
...
// plus the whole renderer/tuning cvar catalog (r_*, combat_log/ai_log, grip_*,
// CSM, wetness, boot_budget_ms, ...). MOVED to app/engine_console.cpp
// (D-CONSOLE fold) so HostShell (app/world_hosts/host_shell.cpp) can register
// the SAME catalog on every --world host's console instead of drifting a
// hand-copy (NO_SLOP rule 4) — see app/engine_console.h. This forwarder keeps
// every existing call site (registerViewmodelCVarsForTest, geolod_shot.cpp,
// screenshot_hosts.cpp, self_tests.cpp) working unchanged.
void registerViewmodelCVars(x3::con::IConsole& console) {
    x3::game::registerEngineConsoleCVars(console);
}
...
int runDefaultHost(HostContext& hc) {
    ...
    // ---- DOOM-style cheat console commands (playtest aid) + noclip + the
    // renderer catalog's paged 'help', ALL through the shared registry
    // (app/engine_console.h — D-CONSOLE fold). HostShell registers the SAME
    // set on every --world host's console via x3::game::registerEngineConsole();
    // the hooks below are the campaign's live wiring (Player / Arsenal /
    // canon-play / chat-tree story flags) — byte-identical bodies to what
    // used to be registered inline here, just relocated behind std::function.
    // 'flightmode', 'restart', and 'help' are registered generically (no
    // hook needed) by the shared function itself.
    bool introPlayRequest = false;
    x3::game::EngineConsoleHooks consoleHooks{};
    consoleHooks.setNoclip = [&player](bool on) {
        player.setNoclip(on);
        if (on && !player.god()) player.setGod(true);   // don't take env damage while flying
    };
    consoleHooks.getNoclip = [&player]() { return player.noclip(); };
    consoleHooks.setGod    = [&player](bool on) { player.setGod(on); };
    consoleHooks.getGod    = [&player]() { return player.god(); };
    consoleHooks.healPlayer = [&player]() { player.heal(); };
    consoleHooks.armAllWeapons = [&game, &canonPlay, &scene]() {
        game.cheatArm(scene);
        if (canonPlay.built()) canonPlay.cheatArm(scene);   // --world canonlevel sidearm
    };
    consoleHooks.setInfiniteAmmo = [&arsenal](bool on) { arsenal.setInfiniteAmmo(on); };
    ...
    consoleHooks.vigilLink = [&chatTrees](bool on) {
