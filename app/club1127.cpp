...
#include "lns_shop.h"      // the LNS shop material kit (CMU walls / checker floor /
                           // neon sign / MR texels) — MOVED there so the tunnel
                           // vehicle bay and this club dress from ONE authored source
...
// ============================================================================
// Forwarded to the shared LNS kit's hash (the body MOVED to lns_shop.cpp with
// the CMU wall recipe): one hash, so "the tunnel bay wears the same paint" is
// true at the texel level, not just in a comment.
inline uint32_t clubHash(uint32_t x) { return lns::lnsHash(x); }
...
// ==================================================================
// ★ THE LNS SHOP KIT MOVED OUT (inspx/lnss-garage). makeMr1x1, the painted
// CMU block wall (albedo + normal), and the neon-letter sign baker were
// authored HERE (LNS GARAGE pass, Tim 2026-07-18) and now live in
// app/lns_shop.{h,cpp}, because the tunnel vehicle bay is the SAME shop and
// must dress from the SAME source — not from a copy that drifts. The using-
// declarations keep every call site below byte-identical.
// ==================================================================
using lns::makeMr1x1;
using lns::makeCmuBlockRGBA;
using lns::makeCmuNormalRGBA;
using lns::makeSignRGBA;
...
const Club1127World::Stats& Club1127World::build(Scene& scene, x3::rhi::IRenderDevice& device,
    ...
    SurfaceSet sWall;
    {
        ...
        auto cmrPx = makeMr1x1(lns::kCmuRoughPx, lns::kCmuMetalPx);   // matte painted block (205/0)
    ...
    auto mrWetPx = makeMr1x1(lns::kFloorRoughPx, lns::kFloorMetalPx);   // (8/255) — the shared kit's number
    ...
    // ==================================================================
    {
        ...
        for (int gx = 0; gx < cols; ++gx)
            for (int gz = 0; gz < rows; ++gz) {
                ...
                // The bright/dark split now reads from the SHARED shop kit
                // (lns_shop.h kCheckerBright/kCheckerDark) — the tunnel bay
                // bakes its floor from the same two numbers, so re-tuning the
                // checker re-tunes BOTH rooms or neither.
                const float* tileCol = lit ? lns::kCheckerBright : lns::kCheckerDark;
