...
inline bool runGroundingSelfTest() {
    ...
    // ---- G12: THE WARD ASSAULT TABLEAU — a PAIR, anchored to a prop ----------
    ...
    //     what makes the nose-clip impossible instead of merely unlikely.
    {
        ...
        const x3::phys::Vec3 cap = groundSceneCharacter(*physics, capWanted, 0.0f,
                                                        "G12/captive", "grounding.h:G12");
        const x3::phys::Vec3 atk = groundSceneCharacter(*physics, atkWanted, 0.0f,
                                                        "G12/attacker", "grounding.h:G12");
        gcheck(approx(cap.y, floorY, 0.02f),
               "G12 the captive stands on the ROOM FLOOR (0.150), not on the cart");
        gcheck(approx(atk.y, floorY, 0.02f),
               "G12 her attacker stands on the ROOM FLOOR (0.150) too");
        gcheck(cap.y < cartTopY - 0.5f,
               "G12 neither participant is anywhere near the cart TOP (0.850)");
        ...
        gcheck(approx(handY, cartTopY, 0.03f),
               "G12 her hands (feet + 0.85) land ON the cart top — the pose's support contract");
        ...
        gcheck(sep >= 0.80f && sep <= 1.40f,
               "G12 attacker stands at a struggle contact distance behind her (0.8-1.4 m)");
        ...
        gcheck(dot > 0.9f,
               "G12 captive and attacker face the SAME way (across the cart) — never nose to nose");
