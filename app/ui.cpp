...
GameState PauseMenu::update(UiContext& ui, PauseAction& outAction, const PauseRows& rows) {
    ...
    // PANEL HEIGHT — KEEP BOTH (fold wave 3). Trunk and feat/hud-restyle each
    // fixed "the last button hangs outside the glass", from opposite ends:
    //
    //   TRUNK pinned the panel to the OLD NUMBER. The screen used to be a fixed
    ...
    //   by exactly one row pitch per extra row, and for the 6-row campaign set
    //   the arithmetic cancels back to `refPh` EXACTLY — the campaign pause
    //   screen stays pixel-identical to what --screenshot-menu has always
    //   captured. It also handles ANY row count (rows.count()) and gives a hint
    //   line its own band. That is the general solution and it is kept whole.
    //
    //   HUD-RESTYLE added the thing trunk has no answer for: a SCREEN BUDGET.
    //   Grown-by-row-pitch still runs off a short window eventually. Its clamp
    //   is kept as a guard — the rows shrink only once the block would exceed
    //   90% of screen height, which on any normal screen at the campaign's six
    //   rows never fires, so the golden capture is untouched.
    const float refPh = std::min(560.0f, h * 0.86f);
    const float refBh = std::max(38.0f, refPh * 0.115f);
    const float refGap= refBh * 0.26f;
    // hud-restyle's TRUE-LEADING fix applies here too: a title band measured at
    // cap height overprints the first row's descenders. hudLineH is 1.5x, and
    // because footH is defined as refPh MINUS the head+rows, the total ph is
    // unchanged — the head grows and the leftover shrinks by exactly as much.
    const float headH = 24.0f + x3::game::hudLineH(titlePx) + 22.0f;
    const float footH = refPh - (headH + 6.0f * refBh + 5.0f * refGap);  // today's leftover
    ...
    float bh = refBh, gap = refGap;
    auto blockH = [&]() {
        return headH + (float)n * bh + (float)(n - 1) * gap + footH + hintBand;
    };
    const float budgetH = h * 0.90f;
    if (blockH() > budgetH) {
        const float rowsH = (float)n * refBh + (float)(n - 1) * refGap;
        const float k = (budgetH - headH - footH - hintBand) / std::max(1.0f, rowsH);
        bh = std::max(28.0f, refBh * k); gap = std::max(4.0f, refGap * k);
    }
