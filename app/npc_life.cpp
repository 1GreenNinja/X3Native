...
// --- Self-contained FREEWAY ribbon. The living-city payload rode terrain.cpp's
// worldFreewaySampleArc()/worldFreewayHalfWidth(), which were tied to that branch's
// stale world topology (a topo::kRoute polyline that no longer exists on the canonical
// line). Grafting the district its OWN straight east-west ribbon, derived from the
// config center, keeps freeway traffic riding into/out of the drag + recycling at the
// bounds — the same feature, with zero coupling to the (now river-only) terrain. ---
constexpr float kFreewayHalfWidth = 8.0f;
inline bool sampleFreewayArc(float centerX, float centerZ, float groundY,
                             float arc, float outCenter[3], float outTangent[2]) {
    arc = arc < 0.0f ? 0.0f : (arc > 1.0f ? 1.0f : arc);
    const float x0 = centerX - 140.0f, x1 = centerX + 140.0f;
    const float z  = centerZ - 40.0f;            // the ribbon runs south of the main drag
    outCenter[0] = x0 + arc * (x1 - x0);
    outCenter[1] = groundY;
    outCenter[2] = z;
    outTangent[0] = 1.0f; outTangent[1] = 0.0f;  // heading due +X
    return true;
}
...
void NpcLife::build(const NpcLifeConfig& cfg, Scene& scene, x3::rhi::IRenderDevice& device,
    ...
    auto spawnWalker = [&](Archetype arch, int idx) {
        ...
        e.roomId = m_cfg.roomId;                      // PVS-cull with the host district
    ...
    const float halfW = std::max(2.0f, kFreewayHalfWidth);
...
void NpcLife::updateFreeway(NpcAgent& a, float dt) {
    ...
    float c[3], tg[2];
    if (sampleFreewayArc(m_cfg.centerX, m_cfg.centerZ, m_cfg.groundY, a.arc, c, tg)) {
