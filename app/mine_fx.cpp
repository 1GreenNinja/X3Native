...
uint32_t GoldMineWorld::addMouthGlow(Scene& scene, x3::rhi::IRenderDevice& device,
    ...
    // Rotate the pane about +Y by `yaw` (0 => +Z-facing). Rigid rotation keeps the
    // CCW winding, so front-face culling still matches VK_FRONT_FACE_CCW.
    const float cyaw = std::cos(yaw), syaw = std::sin(yaw);
    const float nx = syaw, nz = cyaw;                  // rotated +Z normal
...
uint32_t GoldMineWorld::buildMouthGlow(Scene& scene, x3::rhi::IRenderDevice& device,
                                       float cx, float cy, float cz,
                                       float halfW, float halfH, float yaw) {
    // Glow-only: graft just the authentic EoS arch-glow onto an externally-authored
    // mine mesh. Mark built so a stray build() can't also stamp the box-mine on top.
    m_built = true;
