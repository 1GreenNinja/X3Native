...
struct TestFlags {
    ...
    bool testBedRest = false;     // --test-bedrest (doors-pass: lie -> lock+dark, rise -> restored)
    ...
    bool testRiverRapids = false;     // --test-riverrapids (reach table, flow LUT, foam, door, dt)
    ...
    bool testDriveLayer  = false;     // --test-drivelayer (shared traffic/interchange stand-up)
    ...
    bool testComms = false;     // --test-comms (the ship comms device)
