...
void Rifthub::build(Scene& scene, x3::rhi::IRenderDevice& device,
    ...
    // ===== Per-portal blue CORE lights =====
    // One cool-blue point light at each ring center (kRingY high) that casts the
    // event-horizon glow onto the grey stone ring. Base intensity here; tick()
    // pulses it with the core hum. Range comfortably wraps the gate + plate.
    m_lights.clear();
