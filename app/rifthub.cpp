...
void Rifthub::build(Scene& scene, x3::rhi::IRenderDevice& device,
    ...
    for (uint32_t i = 0; i < kPortalCount; ++i) {
        ...
        // local +Y = radial-out (THICK), local +Z = outward (DEEP). Grey stone,
        // authored once at a faint self-lift — the ring is never animated.
        const uint32_t ringEntFirst = scene.size();
        {
            const float halfTang = segHalfTangent(kRingR);
            for (uint32_t s = 0; s < kRingSegments; ++s) {
                ...
                const float segCX = cx     + kRingR * ct * rightX;           // right axis x-component
                const float segCY = kRingY + kRingR * st;                    // up axis is world +Y
                const float segCZ = cz     + kRingR * ct * rightZ;

                // Local +X (tangent), +Y (radial-out), +Z (outward/depth).
                ...
                AddedEntity ae = addOrientedEmissiveBox(
                    scene, device,
                    halfTang, kRingHalfRad, kRingHalfDepth,
                    locX, locY, locZ,
                    segCX, segCY, segCZ,
    ...
    x3::logInfo("[rifthub] hub built with " + std::to_string(m_portals.size()) +
                " Stargate-style portals (thick stone ring, " + std::to_string(kRingSegments) +
                " deep segments + " + std::to_string(kChevronCount) +
