...
// ACES-safe: a bright small hot-spot, but NOT so hot it blooms to a white slab
// over the whole opening (X3_WORLD_RULES rule 5 — flat emissive >~0.5 clips white;
// keep the blue hue by holding luminance down + letting the small disk stay the
// only near-white element).
constexpr float    kCoreBlueMinEm     = 1.6f;           // core blue pulse min
constexpr float    kCoreBlueMaxEm     = 3.2f;           // core blue pulse max (bright)
...
// Pool gradient: bright-blue center -> deep blue rim. Kept BLUE (not near-white)
// so ACES tonemapping preserves the hue instead of desaturating the whole
// membrane to a white disc (X3_WORLD_RULES rule 5). The single near-white element
// is the small core hot-spot disk, not the entire pool.
constexpr float    kPoolCenterBlue[3] = { 0.40f, 0.70f, 1.00f };
constexpr float    kPoolRimBlue[3]    = { 0.06f, 0.28f, 0.95f };
...
constexpr float    kSwirlAmp          = 0.22f;           // swirl emissive contribution (subtle)
...
// ACES-retuned DOWN from 5.5/2.4 — at those strengths the near-white membrane
// clipped to a flat white slab (rule 5), erasing the ripple + blue fresnel
// gradient. These lower strengths keep the pool a bright BLUE liquid whose
// travelling crests + rim actually read through the tonemap.
constexpr float    kPoolBaseEmCenter  = 1.55f;           // innermost band base strength
constexpr float    kPoolBaseEmRim     = 0.85f;           // outermost band base strength
constexpr float    kPoolRippleAmp     = 0.55f;           // travelling-crest amplitude
constexpr float    kPoolMinEm         = 0.30f;           // trough clamp (never fully dark — glow floor)
...
constexpr float    kFresnelRimEm      = 1.05f;           // extra emissive on the rim band (ACES-safe blue lift)
constexpr float    kFresnelShimmerHz  = 0.9f;           // rim shimmer rate
constexpr float    kFresnelShimmerAmp = 0.40f;           // rim shimmer amplitude
...
constexpr float    kKawooshPeakEm     = 3.8f;            // peak added emissive at the flash (ACES-safe surge — a bright bloom, not a total white-out)
