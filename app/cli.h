...
struct CliOptions {
    bool smoketest = false, testAsset = false, testConsole = false, testEngineConsole = false, testPhysics = false,
         ...
         // --test-grounding: character feet vs support surface (app/grounding.h GATE).
         // The gate was written with a --test-grounding flag named in its own header
         // but never plumbed to one, so the flag silently fell through to a normal
         // boot and the gate had never actually run. Both wirings now stand — see the
         // parse site in cli.cpp.
         testGrounding = false,
         testStairNav = false,
    ...
    bool        testDriveLayer  = false;   // --test-drivelayer (the shared traffic/interchange stand-up)
