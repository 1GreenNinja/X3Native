...
std::vector<uint8_t> makeSignRGBA(uint32_t w, uint32_t h, const char* line,
    ...
    auto glyph = [](char c, uint8_t rows[7]) -> bool {
        ...
        switch (c) {
            ...
            // ---- W-FACTORY: the rest of the alphabet. The table shipped with
            // exactly the 16 glyphs the LNS signs happened to spell, and the
            // default case silently renders an unknown letter as a BLANK CELL —
            // so the next sign to want an M would have baked "GLI VALE" and the
            // only tell would have been eyes on the render. Filling the gaps
            // costs nothing and closes the trap. (5x7, MSB = leftmost column.)
            case 'M': set(0x11,0x1B,0x15,0x15,0x11,0x11,0x11); return true;
            case 'V': set(0x11,0x11,0x11,0x11,0x11,0x0A,0x04); return true;
            case 'W': set(0x11,0x11,0x11,0x15,0x15,0x1B,0x11); return true;
            case 'O': set(0x0E,0x11,0x11,0x11,0x11,0x11,0x0E); return true;
            case 'R': set(0x1E,0x11,0x11,0x1E,0x14,0x12,0x11); return true;
            case 'K': set(0x11,0x12,0x14,0x18,0x14,0x12,0x11); return true;
            case 'Y': set(0x11,0x11,0x0A,0x04,0x04,0x04,0x04); return true;
            case 'F': set(0x1F,0x10,0x10,0x1E,0x10,0x10,0x10); return true;
            case 'J': set(0x07,0x02,0x02,0x02,0x02,0x12,0x0C); return true;
            case 'Q': set(0x0E,0x11,0x11,0x11,0x15,0x12,0x0D); return true;
            case 'X': set(0x11,0x11,0x0A,0x04,0x0A,0x11,0x11); return true;
            case 'Z': set(0x1F,0x01,0x02,0x04,0x08,0x10,0x1F); return true;
            case '0': set(0x0E,0x11,0x13,0x15,0x19,0x11,0x0E); return true;
            case '3': set(0x1F,0x02,0x04,0x02,0x01,0x11,0x0E); return true;
            ...
            case '.': set(0x00,0x00,0x00,0x00,0x00,0x0C,0x0C); return true;
