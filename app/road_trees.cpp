...
bool RoadTrees::build(x3::rhi::IRenderDevice& device, const TunnelRoute& route,
    ...
    // NO base-colour textures were published with these packs (the oak has
    // zero images; the poplar carries only a bark normal map), so the glTF
    // default 0.8 grey is all they have. Flat-tint each species a foliage
    // green + matte roughness so they read as trees, not concrete. The
    // material names are per-species, so the rules cannot cross-hit.
    {
        std::vector<EnvArtSystem::MaterialOverride> ovr(2);
        ovr[0].nameSub = "m_hq_oaktree01";
        ovr[0].setBaseColor = true;
        ovr[0].baseColor[0] = 0.21f; ovr[0].baseColor[1] = 0.28f;
        ovr[0].baseColor[2] = 0.12f; ovr[0].baseColor[3] = 1.0f;
        ovr[0].setRoughness = true; ovr[0].roughness = 0.85f;
        ovr[1].nameSub = "m_poplartree001";
        ovr[1].setBaseColor = true;
        ovr[1].baseColor[0] = 0.24f; ovr[1].baseColor[1] = 0.32f;
        ovr[1].baseColor[2] = 0.14f; ovr[1].baseColor[3] = 1.0f;
        ovr[1].setRoughness = true; ovr[1].roughness = 0.85f;
        m_art.setMaterialOverride(std::move(ovr));
    }
