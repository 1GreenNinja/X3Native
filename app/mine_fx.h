...
namespace x3::game { class SurfaceLibrary; }
...
public:
    ...
    // The metal-look family: one build path, ore type retints the mouth glow,
    // the interior lights and the ore props (EoS metal-look.ts table).
    static constexpr int kOreGold    = 0;
    static constexpr int kOreCopper  = 1;
    static constexpr int kOreStone   = 2;
    static constexpr int kOreUranium = 3;
    void setOre(int ore) { m_ore = ore; }
    int  ore() const { return m_ore; }

    // The pooled point lights this mine wants fed to setPointLights (several
    // per mouth: staggered tunnel string + the portal lantern + apron spill).
    // Valid after build().
    const std::vector<x3::rhi::PointLight>& pointLights() const { return m_lights; }

    // Per-frame flame flicker on the tunnel string / lantern (no-op if the
    // scene has no animated entities). Deterministic, dt-scaled.
    void update(float dt, Scene& scene);

    // Optional shared surface library (host-owned) — otherwise the mine mounts
    // its own on first build (assetRoot()/surface_library).
    void setSurfaceLibrary(SurfaceLibrary* s) { m_surf = s; }
    ...
    // Build ONLY the golden mouth-glow quad (no berm/timber/rails) into `scene`
    // at world (cx,cy,cz), facing outward along `yaw` (radians about +Y; 0 => the
    // quad faces +Z), half-extents (halfW,halfH) in metres. For grafting the
    // authentic EoS arch-glow onto an externally-authored mine mesh — e.g.
    // echotropolis's Blender mine_site.glb. Returns the Scene entity id.
    uint32_t buildMouthGlow(Scene& scene, x3::rhi::IRenderDevice& device,
                            float cx, float cy, float cz,
                            float halfW, float halfH, float yaw = 0.0f);
...
private:
    ...
    uint32_t addMouthGlow(Scene& scene, x3::rhi::IRenderDevice& device,
                          float cx, float cy, float cz, float halfW, float halfH, float yaw = 0.0f);

    struct KitImpl;   // .cpp-private Armory kit state

    bool  m_built = false;
    int   m_ore   = kOreGold;
    Stats m_stats{};
    std::vector<x3::rhi::PointLight> m_lights;
    std::vector<MeshLodChain>        m_chains;      // owned LOD chains (Armory meshes)
    SurfaceLibrary* m_surf = nullptr;               // optional shared PBR sets
    uint32_t m_lanternEnt = 0xFFFFFFFFu;            // flicker target
    float    m_flickerT = 0.0f;
