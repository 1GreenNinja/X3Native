...
## 6. Gates
- `--test-elevator` E1-E11 (E1-E6 byte-identical vs main), `--test-elevatorfsm`, `--test-factory` F1-F16 + shell checks (28/28 — Phase 5 added F15b texture-cache steady-state + F16/F16b/F16c hero-hook branch checks; F1c's Sorting/Tube prop pins flex by the recorded hero prim counts), `--test-rifthub` / `--test-city` / `--test-vehicle` regression-clean.

### 6.1 Phase-5 art pass (this branch, feat/factory-annex)
- Curated `fa_*` surface sets (tools/tex_curate_factory.py, 1024^2): fa_iron_wall (corrugated shell), fa_copper_aged (vats/pipes), fa_brass_worn (all trim/rails/torus), fa_enamel_cream (machine bodies + sign strokes), fa_wood_planks (footbridges), fa_tile_checker (Fizz floor overlay + Invention inlay), fa_marble_white (burst dais annulus), fa_fabric_pad (padded room). Conveyor slats ride the existing sr_rubberfloor.
- Glimvale dressing (assets/converted_glb/Glimvale, 13 curated GLBs, dedielectric'd): bank flowers/sunflowers, arches (sandstone-tinted) at room walks, crate/barrel corners (with catch physics), bridge lamps, timber wall sections slotted between the north/south brass columns. The kit's grass/wheat cards are UNTINTED alpha masks (Unity shader-tinted) — excluded, like SM_Stylized_Basket.
- StarForge load-if-present hero hooks (17): 6x `FactoryProps/CopperVat.glb`, 8x `FactoryProps/Machine_<Slug>.glb` (GumStretcher, FizzCompressor, IdeaBellows, SprocketFountain, WobbleBoiler, ButtonOrgan, NotionCentrifuge, MaybeMachine), 2x `FactoryProps/SorterArm.glb` (pivot at origin, arm +X), 1x `FactoryProps/PneumaticCapsule.glb` (origin at center, travel axis +Y). Missing GLB -> procedural fallback + one WARN per hook site with the exact probed path; delivered GLB -> auto-appears on rebuild, zero code change (animated heroes re-pose obj * baked nodeTransform per drawable).
- Emissive law: warm accents <= 0.45 steady / cool <= 0.9; transient event pulses (hatch arming, chase leads) may briefly exceed. The river under-glow is 8 narrow ribbons IN the flow, never a lightbox.
- River water: near-flat heavy syrup (steep Gerstner facets mirrored the sky and rendered bubblegum); deep/shallow are pre-tonemap-calibrated (~0.06 linear reads as deep plum on screen); horizonColor hands the far sea to cocoa.
