    ...
    if(lastElt == firstElt+1)
    {
        if(buildForDither)
        {
            ...
            if( firstElt == 1 )
            {
                ...
                {
                    r = (uint32_t)GifIMin((int)r, image[ii * 4 + 0]);
                    g = (uint32_t)GifIMin((int)g, image[ii * 4 + 1]);
                    b = (uint32_t)GifIMin((int)b, image[ii * 4 + 2]);
                }
            ...
            if( firstElt == (1 << pal->bitDepth)-1 )
            {
                ...
                for(int ii=0; ii<numPixels; ++ii)
                {
                    r = (uint32_t)GifIMax((int)r, image[ii * 4 + 0]);
                    g = (uint32_t)GifIMax((int)g, image[ii * 4 + 1]);
                    b = (uint32_t)GifIMax((int)b, image[ii * 4 + 2]);
                }
...
void GifWriteLzwImage(FILE* f, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal)
{
    ...
    for(uint32_t yy=0; yy<height; ++yy)
    {
        for(uint32_t xx=0; xx<width; ++xx)
        {
            ...
            //int32_t lookupValue = nextValue;
            //GifWriteCode( f, stat, lookupValue, codeSize );
