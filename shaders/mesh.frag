...
// PBR slice 1: bindless map indices. Placed at 11/12 because mesh.vert's
// glass material outputs occupy 8/9/10 (consumed by glass.frag, not us).
layout(location = 11) flat in uint vNormalTexIndex; // 0 = none (PBR normal map)
layout(location = 12) flat in uint vMrTexIndex;     // 0 = none (metallic-roughness)
