...
// Lens-flare HDR buffer (binding 4): generated by lens_flare.frag this frame
...
// COMPOSITE-INTRO UNION: god-rays (broad scatter, binding 3) is added FIRST, then
// lens flare (sharp ghosts/streaks, binding 4) on top — both additive, BEFORE ACES.
layout(set = 0, binding = 4) uniform sampler2D lensTex;    // lens-flare (half res)
...
void main() {
    ...
    // COMPOSITE-INTRO UNION (both additive, in linear light, BEFORE exposure+tonemap):
    // god-rays FIRST (broad volumetric scatter), then lens flare ON TOP (sharp
    // ghosts/streaks sit over the shafts). Each guarded independently so r_godrays 0
    // / r_lensflare 0 never samples its target -> that off-path stays byte-identical
    // to the pre-effect build; both off == base.
    // Additive god-rays / light shafts.
    if (pc.godraysIntensity > 0.0)
        color += texture(godraysTex, vUV).rgb * pc.godraysIntensity;
    // Lens flare: ghosts/halo/streak/sun-flare, ACES then rolls the extra radiance off.
