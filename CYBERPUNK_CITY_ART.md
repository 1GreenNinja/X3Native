...
### A2) SCENE LAYOUT — reconstruct the DESIGNERS' city (the big win, per 14900K)
The pack ships the designers' fully-arranged city as **Unity YAML** you can parse to JSON: **2 `.unity` demo scenes + 331 `.prefab`s** under `…\Cyberpunk City … Sci-Fi City\_Layouts\`. These hold the EXACT placement (mesh ref + position/rotation/scale) of every building/prop — so reproduce the designed cityscape **1:1** instead of hand-placing.
Pipeline: parse the `.unity` scene(s) (+ referenced `.prefab`s) → resolve each `GameObject`'s mesh via the `.meta` **GUID → FBX** map → emit a **placement manifest** (glb name + transform). Unity is Y-up left-handed; X3Native is Y-up right-handed RH — apply the standard axis fix (negate Z or flip handedness consistently) + Unity's 1 unit = 1 m. Watch prefab nesting (compose child transforms). Ship the manifest as a data file (JSON / `.x3lvl`) the overlay loads.

### B) CityArt OVERLAY — `app/city_art.{h,cpp}`
A new `CityArtSystem` modelled on `env_art.{h,cpp}`: load the converted Cyberpunk GLBs + place instances **at the parsed scene-layout manifest transforms (A2)** — the designer-faithful city — anchored onto `city.cpp`'s district region (its `CityZonePlan` center/footprint) so it lines up with the graybox + roads/tunnels (read `City`'s queries — do NOT edit `city.*`). Where no scene data covers a spot, fall back to procedural placement on the district footprints. Tower buildings on the district blocks, neon signage + emissive strips (HDR bloom — `Entity::emissive`), street props + parked cyber-cars along the roads, tunnel facades. Aim for a night-time **neon Cyberpunk-2077** read (lean on emissive + the existing bloom). Per-piece fallback (missing GLB → graybox stays). Do NOT touch `city.*`, `world_regions.*`, `act2_*`, `monster.*`.
