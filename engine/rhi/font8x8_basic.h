#pragma once
// ---------------------------------------------------------------------------
// font8x8_basic — public-domain 8x8 monospace bitmap font (ASCII 0x00..0x7F).
//
// SOURCE / LICENSE: This is the well-known "font8x8" public-domain bitmap font.
// The 8x8 ASCII glyph set originates with the IBM PC ROM-style fonts and was
// re-typed and released into the PUBLIC DOMAIN by Marcel Sondaar (the "IBM PC
// 8x8 font" used in dhepnix/JdeRobot work) and collected by Daniel Hepper as
// `font8x8` (https://github.com/dhepper/font8x8) under a public-domain
// dedication. No GPL / id Tech / RBDOOM source was consulted — only this
// public-domain ASCII glyph data, embedded here directly.
//
// FORMAT: 128 entries, one per ASCII code. Each entry is 8 bytes = 8 rows
// (row 0 = top). Within a row, bit 0 (LSB, value 1) is the LEFTMOST pixel and
// bit 7 (MSB, value 128) is the RIGHTMOST pixel. A set bit means an "on" pixel.
// ---------------------------------------------------------------------------
#include <cstdint>

namespace x3::rhi {

// 128 glyphs * 8 rows. Codes 0x00..0x1F are blank (control chars).
