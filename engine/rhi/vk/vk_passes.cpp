...
void VulkanRenderDevice::drawMeshGlass(const FrameContext& fc, MeshHandle mesh, TextureHandle baseColor,
        ...
        float factor[4] = {
            ...
            glass.opacity };
        // BLEND-partition routing for glass. Two independent reasons a glass draw
        // must ride the BLEND record tail instead of the opaque range:
        //   (1) caller opt-in via `alphaBlend` (default false) — a translucent shell
        //       IN FRONT of an opaque body you must still see through (e.g. a sun's
        //       corona over its core). See IRenderDevice::drawMeshGlass.
        //   (2) ADDITIVE GLOW glass (GlassMaterial::additive > 0 — street-light cones/
        //       pools): an opaque-range record replays in the DEPTH PRE-PASS, whose
        //       written cone depth makes every opaque surface behind it fail the color
        //       pass's EQUAL test — the "cone" then shows the raw sky/clear through a
        //       depth hole (the solid-funnel artifact). Normal glass masks that hole by
        //       outputting alpha=1 from the scene copy; the additive branch (tiny alpha)
        //       does not, so it must never enter the pre-pass/shadow ranges.
