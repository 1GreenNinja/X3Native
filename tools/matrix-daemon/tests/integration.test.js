...
describe('integration: inbox flow', () => {
  ...
  test('simulated matrix room.message event lands a well-formed line in the inbox', () => {
    ...
    const roomId = '!fleet-ops:tims-fleet.xyz';
    const myUserId = '@djbooth:tims-fleet.xyz';
    ...
    expect(stored.room).toBe('!fleet-ops:tims-fleet.xyz');
    expect(stored.sender).toBe('@13700k:tims-fleet.xyz');
    expect(stored.text).toContain('rebase');
    expect(stored.eventId).toBe('$evt:tims-fleet.xyz');
  ...
  test('self-sent events are filtered (daemon-level gate)', () => {
    ...
    const myUserId = '@djbooth:tims-fleet.xyz';
    const selfEvent = {
      event_id: '$self-evt:tims-fleet.xyz',
      sender: myUserId,
      origin_server_ts: 1779950000000,
  ...
  test('multiple events in sequence accumulate in inbox', () => {
    const events = [
      { sender: '@tim:tims-fleet.xyz', text: 'morning all' },
      { sender: '@13700k:tims-fleet.xyz', text: 'morning Tim — branches look clean' },
      { sender: '@14900k:tims-fleet.xyz', text: 'rebasing now, give me 5 min' },
    ];
    ...
    expect(JSON.parse(lines[2]).sender).toBe('@14900k:tims-fleet.xyz');
  });
