...
6. **M-C evictions and M-D spawn-region boot are BLOCKED on the host tick.** Both
   need `host_echotropolis.cpp` (Lane 7's file this session): M-C needs the streamer's
   `update()` called with the camera probe, M-D needs region builders gated on
   residency. Nothing in the engine blocks either — the streamer, the hooks and the
   `EchoRegion` container all exist and boot today. See §7b for what M-D actually buys.
7. GPU work is not a priority at 12.4 ms. If it ever becomes one, the four passes worth
