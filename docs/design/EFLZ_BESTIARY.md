...
### The 4 engine "species" are GENERIC STAND-INS, not designed species
`DominionTrooper`, `Verthani`, `Illuminated`, `BlueSynth` are **engine-invented names** chosen to exercise the 4 AI behavior shapes (baseline-melee / fast-flanker / ranged-standoff-elite / ranged-drone-flier). They are explicitly *mapped onto* bible profiles in code comments, but **none of these four names exist in the EFLZ bible.** Only **Chief Martinez** (Boss) and the **Clone** (used on F7 per design notes) are real EFLZ entities currently represented.
