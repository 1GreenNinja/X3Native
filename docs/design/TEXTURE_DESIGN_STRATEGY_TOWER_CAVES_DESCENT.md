...
The anchors: F7's hidden lift behind the exec desk (trigger 80) descends through SL1→SL2→SL3 to −178 m. The slide is the *unpowered* counterpart — the physical chute the player rides when the lift is denied or as the fast/loud route: **a winding gravity chute from the tower's service core to the Salvari horizon**, intersecting the sub-levels as glimpse-windows on the way down (SL1 steam, SL2 frost-blue flash, SL3 red interrogation glow) so the ~180 m fall previews the spaces the player will fight back UP through. That up-through-what-you-slid-past loop is the level-design thesis of the whole lower game.
